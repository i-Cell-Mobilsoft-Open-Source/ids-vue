...
import { createComponentError } from '@core/utils/CreateError';
  ...
  onMounted(() => {
    ...
    if (items.value.length < minItemCount || items.value.length > maxItemCount ) {
      throw new Error(
        createComponentError(componentClass, 'invalid count of segmented control items. Minimum item count is 2, maximum is 5.'),
      );
    }
...
<template>
  <div :id="segmentedControlId" ref="segmentedControlRef" :class="classObject" :role="setRole">
    <slot />
  </div>
</template>
<style lang="scss">
